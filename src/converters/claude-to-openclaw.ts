import type { ClaudeAgent, ClaudeCommand, ClaudeMcpServer, ClaudePlugin, ClaudeHooks } from "../types/claude"
import type { OpenClawBundle, OpenClawConfig, OpenClawCommandConfig, OpenClawMcpServer, OpenClawAgentConfig } from "../types/openclaw"
import { formatFrontmatter } from "../utils/frontmatter"

export type PermissionMode = "none" | "broad" | "from-commands"

export type ClaudeToOpenClawOptions = {
...
export function convertClaudeToOpenClaw(
  ...
  applyPermissions(config, plugin.commands, options.permissions || "broad")
...
function convertAgent(agent: ClaudeAgent, options: ClaudeToOpenClawOptions) {
...
function convertCommands(commands: ClaudeCommand[]): Record<string, OpenClawCommandConfig> {
  const result: Record<string, OpenClawCommandConfig> = {}
  for (const command of commands) {
    if (command.disableModelInvocation) continue
    const entry: OpenClawCommandConfig = {
...
function convertMcp(servers: Record<string, ClaudeMcpServer>): Record<string, OpenClawMcpServer> {
  const result: Record<string, OpenClawMcpServer> = {}
...
function convertHooks(hooks: ClaudeHooks) {
  ...
  const content = `${unmappedComment}import type { Plugin } from "@openclaw-ai/plugin"\n\nexport const ConvertedHooks: Plugin = async ({ $ }) => {\n  return {\n${handlerBlocks.join(",\n")}\n  }\n}\n\nexport default ConvertedHooks\n`
